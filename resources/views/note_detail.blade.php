        ...
        <div class="title flex justify-between items-center">
            ...
            <div class="relative">
                ...
                <div id="menu" class="origin-top-right absolute right-0 mt-2 w-56 rounded-sm shadow-lg bg-white ring-1 ring-black ring-opacity-[0.04] hidden">
                    <!-- Opsi dropdown -->
                    <a href="{{route('form-update', $note['id'])}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">Edit</a>
                    <a id="delete" data-id="{{$note['id']}}" class="block px-4 py-2 text-sm text-red-700 hover:bg-gray-100 hover:text-red-900" role="menuitem">Delete</a>
            </div>
    ...
    <script>
        ...
        document.getElementById("delete").addEventListener("click", function(event) {
            ...
            if (confirm("Anda yakin ingin menghapus?")) {
                fetch(`{{route('delete-note', $note['id'])}}`, {
                    method: "DELETE",
                    headers: {
                        "X-CSRF-TOKEN": csrf
                    } 
